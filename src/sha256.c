...
/*   Updated: 2018/09/16 15:06:57 by jjourdai         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static uint32_t	g_initial_value[8] = {
...
static void			compute_inter_hash(uint32_t *w, uint32_t *inter_hash)
{
	...
	while (++i < 64)
	{
		temp1 = inter_hash[7] + S1(inter_hash) + CH(inter_hash) + g_k[i] + w[i];
...
{
	uint32_t	inter_hash[8];
	uint64_t	len;
	uint32_t	w[64];
	uint32_t	hash[8];
	uint8_t		*bytes_string;

	ft_memcpy(hash, g_initial_value, sizeof(hash));
	ft_bzero(&w, sizeof(w));
	len = put_padding_character_sha256(info);
	bytes_string = info->bytes;
	while (len > 0)
	{
